...
const EditarBorrador: React.FC = () => {
  ...
  const [tipoMensaje, setTipoMensaje] = useState<TipoMensaje>(TIPOS_MENSAJE.INDIVIDUAL);
  ...
  const cargarBorrador = async () => {
    try {
      ...
      formik.setValues({
        _id: borrador._id,
        destinatarios: borrador.destinatarios.map((d: any) => typeof d === 'string' ? d : d._id),
      ...
      if (borrador.adjuntos && borrador.adjuntos.length > 0) {
        ...
        const adjuntosExistentes: AdjuntoUI[] = borrador.adjuntos.map((adjunto: AdjuntoBackend) => ({
          preExistente: true,
      ...
      setError('No se pudo cargar el borrador. ' + (err.message || 'Intente nuevamente más tarde.'));
    } finally {
  ...
  const guardarBorrador = async () => {
    try {
      ...
      const esValido = await formik.validateForm();
      if (Object.keys(esValido).length > 0) {
        // Hay errores de validación
        formik.setTouched({
          asunto: true,
          contenido: true,
          ...(tipoMensaje === TIPOS_MENSAJE.INDIVIDUAL ? { destinatarios: true } : { cursoId: true }),
          prioridad: true
        });
        setLoading(false);
        return;
      ...
      const borrador: IBorrador = {
        _id: formik.values._id, // Incluir ID si estamos actualizando
        destinatarios: tipoMensaje === TIPOS_MENSAJE.INDIVIDUAL ? formik.values.destinatarios : [],
        asunto: formik.values.asunto,
        contenido: formik.values.contenido,
        prioridad: formik.values.prioridad,
        tipo: tipoMensaje,
      };
      
      // Agregar cursoId si es mensaje grupal
      if (tipoMensaje === TIPOS_MENSAJE.GRUPAL) {
      ...
      // Guardar borrador
      const response = await mensajeService.guardarBorrador(borrador, nuevosAdjuntos);
      ...
      setError('No se pudo guardar el borrador. ' + (err.message || 'Intente nuevamente más tarde.'));
      setSnackbar({
        open: true,
  ...
  const enviarBorrador = async () => {
    try {
      ...
      setError('No se pudo enviar el borrador. ' + (err.message || 'Intente nuevamente más tarde.'));
      setSnackbar({
        open: true,
              ...
              <Grid item xs={12}>
                ...
                <Box sx={{ mb: 2 }}>
                  <Autocomplete
                    ...
                    noOptionsText={query.trim() ? "No se encontraron destinatarios" : "Escriba para buscar"}
                  ...
                  {adjuntos.map((file, index) => (
                    <ListItem key={index}>
                      <ListItemText
                        ...
                        secondary={`${((file.tamaño || file.size) / 1024).toFixed(2)} KB`}
                      />
                      <ListItemSecondaryAction>
                        <IconButton
                          edge="end"
...
>
  ...
  <DialogContent>
    <DialogContentText id="cancel-dialog-description">
      Los cambios no guardados se perderán. ¿Está seguro de que desea salir?
    </DialogContentText>
  </DialogContent>
  <DialogActions>
    ...
    <Button onClick={() => navigate('/mensajes/borradores')} color="error">
      Salir sin guardar
    </Button>
  </DialogActions>
</Dialog>
