...
const FormularioEvento: React.FC = () => {
  ...
  const validationSchema = Yup.object({
    ...
    lugar: Yup.string().when('todoElDia', {
      is: false,
      then: (schema) => schema.required('El lugar es obligatorio para eventos con hora específica').trim(),
    ...
    onSubmit: async (values) => {
      try {
        ...
        try {
          ...
          if (values.todoElDia) {
            // Para eventos de todo el día, usar fechas sin la hora (00:00)
          ...
          setError(`Error con las fechas: ${err.message}`);
          setLoading(false);
          return;
        }
        ...
        const eventoData = {
          ...
          // Si es un rol autorizado, usar el estado seleccionado, si no, usar PENDIENTE
          estado: (user?.tipo === 'ADMIN' || user?.tipo === 'DOCENTE' || user?.tipo === 'ADMINISTRATIVO' || user?.tipo === 'COORDINADOR') 
            ? (formik.values.estado || 'PENDIENTE') 
            : 'PENDIENTE',
          color: values.color || '#3788d8',
        };
        
        console.log('Datos del evento a enviar:', JSON.stringify(eventoData, null, 2));
        console.log('fechaInicio:', fechaInicio.toString());
        console.log('fechaFin:', fechaFin.toString());
        
        if (isEditMode) {
          // Modo edición
          await calendarioService.actualizarEvento(id!, eventoData, archivo || undefined);
          setSuccess('Evento actualizado correctamente');
        } else {
          // Modo creación
          await calendarioService.crearEvento(eventoData, archivo || undefined);
        ...
        if (err.message) {
          ...
          setError('Error al guardar el evento. Verifica que todos los campos sean válidos.');
        }
      } finally {
        setLoading(false);
      }
      ...
      if (savedDate) {
        try {
          ...
          if (!isNaN(fecha.getTime())) {
            ...
            const fechaCorregida = new Date(fecha.getFullYear(), fecha.getMonth(), fecha.getDate(), 12, 0, 0);
            formik.setFieldValue('fechaInicio', fechaCorregida);
            formik.setFieldValue('fechaFin', fechaCorregida);
            
            console.log('Fecha cargada desde sessionStorage:', fechaCorregida.toLocaleDateString());
          }
        } catch (err) {
  ...
  useEffect(() => {
    const cargarEvento = async () => {
      ...
      try {
        ...
        setError(err.message || 'Error al cargar el evento. Intente nuevamente.');
      } finally {
        setInitialLoading(false);
      }
    };
      ...
      >
        <form onSubmit={formik.handleSubmit}>
          <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
              <FormControl component="fieldset">
                ...
                <RadioGroup
                  ...
                  onChange={(e) => {
                    formik.setFieldValue('todoElDia', e.target.value === 'true');
                  }}
                >
                  <FormControlLabel value="false" control={<Radio />} label="Hora específica" />
                  <FormControlLabel value="true" control={<Radio />} label="Todo el día" />
                </RadioGroup>
              </FormControl>
            </Grid>
              ...
              <>
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) => {
                      ...
                      const fechaAjustada = new Date(year, month - 1, day, 12, 0, 0);
                      formik.setFieldValue('fechaInicio', fechaAjustada);
                    }}
                    InputLabelProps={{ shrink: true }}
                    error={formik.touched.fechaInicio && Boolean(formik.errors.fechaInicio)}
                    helperText={formik.touched.fechaInicio && String(formik.errors.fechaInicio)}
                    disabled={loading}
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                    required
                  />
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    helperText={formik.touched.fechaFin && String(formik.errors.fechaFin)}
                    disabled={loading}
                    sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                    required
                  />
            ...
            {/* Permitir seleccionar el estado directamente para usuarios con roles autorizados */}
              ...
              <Grid item xs={12} sm={6}>
                ...
                >
                  ...
                  <Select
                    ...
                    disabled={loading}
                  >
                    <MenuItem value="PENDIENTE">Pendiente (requiere aprobación)</MenuItem>
                    <MenuItem value="ACTIVO">Activo (visible para todos)</MenuItem>
                    <MenuItem value="CANCELADO">Cancelado</MenuItem>
                  </Select>
                  <FormHelperText>
                    Seleccione si el evento estará visible inmediatamente o requiere aprobación.
                  </FormHelperText>
                </FormControl>
              </Grid>
            )}
            ...
            <Grid item xs={12}>
              ...
              >
                {archivo ? 'Cambiar archivo' : archivoActual ? 'Reemplazar archivo' : 'Seleccionar archivo'}
                <input
                  type="file"
                  hidden
