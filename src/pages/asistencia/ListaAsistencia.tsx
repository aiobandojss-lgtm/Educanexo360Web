...
const ListaAsistencia = () => {
  ...
  const [fechaInicio, setFechaInicio] = useState<string>(format(new Date(new Date().getFullYear(), new Date().getMonth(), 1), 'yyyy-MM-dd'));
  ...
  useEffect(() => {
    const cargarCursos = async () => {
      try {
        ...
        setError('No se pudieron cargar los cursos: ' + (err.response?.data?.message || 'Error del servidor'));
      } finally {
  ...
  const cargarAsistencias = async () => {
    try {
      ...
      setError('No se pudieron cargar las asistencias: ' + (err.response?.data?.message || 'Error del servidor'));
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (cursoSeleccionado || (user?.tipo === 'ADMIN')) {
      cargarAsistencias();
    }
  ...
  const calcularEstadisticas = () => {
    ...
    const totalEstudiantes = asistencias.reduce((sum, a) => sum + a.totalEstudiantes, 0);
    ...
    const totalJustificados = asistencias.reduce((sum, a) => sum + a.justificados, 0);
    
    const promedioAsistencia = totalEstudiantes > 0 
      ? Math.round(((totalPresentes + totalJustificados) / totalEstudiantes) * 100) 
      : 0;
    ...
    <Box>
      ...
      {estadisticas && (
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={12} sm={6} md={3}>
            <Card elevation={0} sx={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.05)', height: '100%' }}>
              <CardContent sx={{ textAlign: 'center' }}>
                <Typography variant="h3" gutterBottom>Total Registros</Typography>
                <Typography variant="h1" color="primary.main">{estadisticas.totalRegistros}</Typography>
              </CardContent>
            </Card>
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card elevation={0} sx={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.05)', height: '100%' }}>
              <CardContent sx={{ textAlign: 'center' }}>
                ...
                <Typography variant="h1" color="success.main">{estadisticas.totalPresentes}</Typography>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card elevation={0} sx={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.05)', height: '100%' }}>
              <CardContent sx={{ textAlign: 'center' }}>
                ...
                <Typography variant="h1" color="error.main">{estadisticas.totalAusentes}</Typography>
                <Typography variant="body2" color="text.secondary">
                  {estadisticas.totalEstudiantes > 0 
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card elevation={0} sx={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.05)', height: '100%' }}>
              <CardContent sx={{ textAlign: 'center' }}>
                <Typography variant="h3" gutterBottom>Promedio Asistencia</Typography>
      ...
      {!cursoSeleccionado && user?.tipo === 'ADMIN' && !loadingCursos && (
        <Alert severity="info" sx={{ mb: 3 }}>
          Mostrando datos de todos los cursos. Seleccione un curso específico para filtrar los resultados.
        </Alert>
      )}
                ...
                asistencias
                  ...
                  .map((asistencia) => (
                    <TableRow key={asistencia._id} hover>
                      ...
                      <TableCell>
                        {asistencia.curso.nombre} - {asistencia.curso.grado} {asistencia.curso.seccion}
                      </TableCell>
                      ...
                      <TableCell>
                        {asistencia.registradoPor.nombre} {asistencia.registradoPor.apellidos}
                      </TableCell>
                      <TableCell align="center">
