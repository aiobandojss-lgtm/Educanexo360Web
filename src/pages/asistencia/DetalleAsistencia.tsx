...
import asistenciaService, { ESTADOS_ASISTENCIA } from '../../services/asistenciaService';
...
const DetalleAsistencia = () => {
  ...
  const calcularEstadisticas = () => {
    ...
    const presentes = asistencia.estudiantes.filter((e: any) => e.estado === ESTADOS_ASISTENCIA.PRESENTE).length;
    const ausentes = asistencia.estudiantes.filter((e: any) => e.estado === ESTADOS_ASISTENCIA.AUSENTE).length;
    const tardanzas = asistencia.estudiantes.filter((e: any) => e.estado === ESTADOS_ASISTENCIA.TARDANZA).length;
    const justificados = asistencia.estudiantes.filter((e: any) => e.estado === ESTADOS_ASISTENCIA.JUSTIFICADO).length;
    const permisos = asistencia.estudiantes.filter((e: any) => e.estado === ESTADOS_ASISTENCIA.PERMISO).length;
    
    const porcentajePresentes = totalEstudiantes > 0 ? Math.round((presentes / totalEstudiantes) * 100) : 0;
    const porcentajeAsistencia = totalEstudiantes > 0 
      ? Math.round(((presentes + tardanzas + justificados + permisos) / totalEstudiantes) * 100) 
      : 0;
  ...
  const puedeEditar = ['ADMIN', 'DOCENTE'].includes(user?.tipo || '') && asistencia && !asistencia.finalizado;
  ...
  return (
    <Box>
          ...
          <Grid item xs={12} md={4}>
            <Box sx={{ display: 'flex', alignItems: 'flex-start' }}>
              ...
              <Box>
                ...
                <Typography variant="h3">
                  {asistencia.cursoNombre || 'No especificado'}
                </Typography>
              </Box>
            </Box>
          </Grid>
      ...
      {estadisticas && (
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={12} sm={6} md={3}>
            <Card elevation={0} sx={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.05)', height: '100%' }}>
              <CardContent sx={{ textAlign: 'center' }}>
                ...
                <Typography variant="h1" color="primary.main">{estadisticas.totalEstudiantes}</Typography>
              </CardContent>
            </Card>
          </Grid>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card elevation={0} sx={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.05)', height: '100%' }}>
              <CardContent sx={{ textAlign: 'center' }}>
                <Typography variant="h3" gutterBottom>Presentes</Typography>
                <Typography variant="h1" color="success.main">{estadisticas.presentes}</Typography>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card elevation={0} sx={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.05)', height: '100%' }}>
              <CardContent sx={{ textAlign: 'center' }}>
                <Typography variant="h3" gutterBottom>Ausentes</Typography>
                <Typography variant="h1" color="error.main">{estadisticas.ausentes}</Typography>
                <Typography variant="body2" color="text.secondary">
          ...
          <Grid item xs={12} sm={6} md={3}>
            <Card elevation={0} sx={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.05)', height: '100%' }}>
              <CardContent sx={{ textAlign: 'center' }}>
                <Typography variant="h3" gutterBottom>Asistencia</Typography>
                <Box sx={{ position: 'relative', display: 'inline-flex' }}>
                  <CircularProgress 
                    ...
                    thickness={7}
                    sx={{ 
                      color: estadisticas.porcentajeAsistencia >= 90 
                        ? 'success.main' 
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            ¿Está seguro de eliminar este registro de asistencia? Esta acción no se puede deshacer.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button 
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Al finalizar el registro, se confirmará que la información es correcta y ya no se podrán realizar cambios.
            ¿Desea finalizar el registro de asistencia?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button 
          ...
          <Button 
            ...
            autoFocus
            disabled={procesando}
            startIcon={procesando ? <CircularProgress size={24} /> : <FinalizarIcon />}
          >
