...
const RegistroAsistencia: React.FC = () => {
  ...
  const formik = useFormik<IRegistroAsistencia>({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const todosConEstado = values.estudiantes.every(e => e.estado);
        if (!todosConEstado) {
        ...
        if (modoEdicion) {
          ...
          await asistenciaService.actualizarRegistroAsistencia(id!, values);
        } else {
          // Crear nuevo registro
          await asistenciaService.crearRegistroAsistencia(values);
        }
        ...
        setError(err.response?.data?.message || 'Error al guardar el registro de asistencia');
      } finally {
        setGuardando(false);
      }
    },
    ...
    if (modoEdicion && id) {
      const cargarRegistro = async () => {
        try {
          ...
          // Cargar dependencias (cursos, asignaturas, estudiantes)
          // Se cargarán automáticamente por los otros useEffect cuando cambie cursoId
        } catch (err: any) {
  ...
  useEffect(() => {
    ...
    const cargarAsignaturas = async () => {
      try {
        setLoading(true);
        const data = await asistenciaService.obtenerAsignaturasPorCurso(formik.values.cursoId);
        setAsignaturas(data);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const cargarEstudiantes = async () => {
      try {
        setLoading(true);
        const data = await asistenciaService.obtenerEstudiantesPorCurso(formik.values.cursoId);
        setEstudiantes(data);
        ...
        if (!modoEdicion || formik.values.estudiantes.length === 0) {
          ...
          const estudiantesAsistencia: EstudianteAsistencia[] = data.map((est: any) => ({
  ...
  return (
    <Box>
      <Typography variant="h2" color="primary.main" gutterBottom>
        {modoEdicion ? 'Editar Registro de Asistencia' : 'Nuevo Registro de Asistencia'}
      </Typography>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                <TextField
                  ...
                  label="Fecha"
                  type="date"
                  value={format(formik.values.fecha instanceof Date ? formik.values.fecha : new Date(), 'yyyy-MM-dd')}
                  onChange={(e) => {
              ...
              <Grid item xs={12} md={6}>
                ...
                >
                  ...
                  <Select
                    ...
                    label="Curso"
                    disabled={loading || guardando || (modoEdicion && formik.values.estudiantes.length > 0)}
                  >
                    {cursos.map((curso) => (
              ...
              <Grid item xs={12} md={4}>
                <FormControl fullWidth>
                  <InputLabel id="asignatura-label">Asignatura (opcional)</InputLabel>
                  <Select
              ...
              <Grid item xs={12} md={4}>
                <FormControl 
                  fullWidth 
                  error={formik.touched.tipoSesion && Boolean(formik.errors.tipoSesion)}
                >
              ...
              <Box sx={{ mt: 4 }}>
                ...
                <Box sx={{ mt: 2, mb: 2, display: 'flex', gap: 1 }}>
                  <Button
                    ...
                    onClick={() => handleEstadoMasivo(ESTADOS_ASISTENCIA.PRESENTE)}
                    disabled={guardando}
                    size="small"
                  >
                    Todos Presentes
                  ...
                  <Button
                    ...
                    onClick={() => handleEstadoMasivo(ESTADOS_ASISTENCIA.AUSENTE)}
                    disabled={guardando}
                    size="small"
                  >
                    Todos Ausentes
                  ...
                  <Table>
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell width="30%" align="center">Estado</TableCell>
                        ...
                        <TableRow key={estudiante.estudianteId}>
                          ...
                          <TableCell align="center">
                            <EstadoAsistenciaSelector
                              ...
                              onChange={(estado) => handleEstadoChange(estudiante.estudianteId, estado)}
                              disabled={guardando}
                            />
                          </TableCell>
                          <TableCell>
                            <TextField
                              ...
                              onChange={(e) => handleObservacionesChange(estudiante.estudianteId, e.target.value)}
                              disabled={guardando}
                            />
                          </TableCell>
                        </TableRow>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={() => setConfirmDialog(false)}>No, continuar editando</Button>
          <Button 
