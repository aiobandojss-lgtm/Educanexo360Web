...
const buscarDestinatariosParaAcudiente = async () => {
  try {
    ...
    const response = await axiosInstance.get('/mensajes/destinatarios-acudiente');
    return response.data.data || [];
  } catch (error: any) {
    console.error('[Frontend] Error buscando destinatarios para acudiente:', error);
...
const buscarDestinatarios = async (query = '', isAcudiente = false) => {
  try {
    console.log(`[DEBUG] Buscando destinatarios con query: "${query}", isAcudiente: ${isAcudiente}`);
    ...
    // Llamar al endpoint
    const response = await axiosInstance.get(`/mensajes/destinatarios-disponibles?${params.toString()}`);
    return response.data.data || [];
  } catch (error: any) {
    ...
    if (error.response && error.response.status === 403) {
      console.log('Usuario con permisos restringidos para listar estos destinatarios');
    }
...
const enviarMensaje = async (mensaje: MensajeRequest, adjuntos: File[] = []) => {
  try {
...
const responderMensaje = async (
  ...
  try {
    ...
    if (!mensajeOriginal || !mensajeOriginal.remitente) {
      throw new Error('No se pudo obtener la información del remitente original');
    }
...
const obtenerCursosDisponibles = async () => {
  try {
    ...
    if (cursos.length > 0) {
      console.log('[DEBUG] Ejemplo de estructura de curso:', 
        JSON.stringify({
          id: cursos[0]._id,
...
const archivarMensaje = async (id: string, bandeja?: string) => {
  try {
    ...
    const url = `/mensajes/${id}/archivar${params.toString() ? `?${params.toString()}` : ''}`;
    logRequest('PUT', url);
...
const eliminarDefinitivamente = async (id: string) => {
  try {
    ...
    console.error('[Frontend] Error eliminando definitivamente mensaje:', error);
    throw error;
  }
};
...
const guardarBorrador = async (
  ...
  try {
    ...
    const endpoint = isUpdate ? `/mensajes/borradores/${borrador._id}` : '/mensajes/borradores';
    
    // Si no hay adjuntos, usar JSON
    if (adjuntos.length === 0) {
      logRequest('POST', endpoint, borrador);
      const response = await axiosInstance.post(endpoint, {
        ...borrador,
        tipo: borrador.tipo || (borrador.cursoId ? TipoMensaje.GRUPAL : TipoMensaje.INDIVIDUAL)
      });
      return response.data;
    }
    ...
    // Agregar todos los campos del borrador
    Object.entries(borrador).forEach(([key, value]) => {
      // Saltar id si es undefined
      if (key === '_id' && !value) return;
      
      // Manejar array de destinatarios
      if (key === 'destinatarios' && Array.isArray(value)) {
        value.forEach(dest => formData.append('destinatarios', dest));
      } else {
        // Añadir otros campos
    ...
    if (!borrador.tipo) {
      formData.append('tipo', borrador.cursoId ? TipoMensaje.GRUPAL : TipoMensaje.INDIVIDUAL);
    }
...
const marcarComoLeido = async (id: string, leido: boolean) => {
  try {
    console.log(`Intentando marcar mensaje ${id} como ${leido ? 'leído' : 'no leído'}`);
    ...
    try {
      ...
      console.log('[DEBUG] Mensaje marcado correctamente como', leido ? 'leído' : 'no leído');
      return response.data;
    } catch (error: any) {
    ...
    console.error('[Frontend] Error marcando mensaje como leído/no leído:', error);
    throw error;
  }
};
...
const obtenerMensajes = async (
  ...
  try {
    ...
    if (response.data && response.data.data && Array.isArray(response.data.data) && userId) {
      // Función auxiliar para verificar el estado del mensaje para un usuario
      const verificarEstado = (mensaje: MensajeConEstados, estado: string): boolean => {
        if (!mensaje.estadosUsuarios || !Array.isArray(mensaje.estadosUsuarios)) {
          return false;
        }
      ...
      response.data.data = response.data.data.filter(
        (mensaje: MensajeConEstados) => {
          ...
          return !estadoUsuario || estadoUsuario.estado !== 'ELIMINADO_PERMANENTE';
        }
      );
